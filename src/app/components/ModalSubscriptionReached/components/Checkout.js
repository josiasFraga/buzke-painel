...
const Content = styled.div`
    ...
    label:not(.invalid-feedback) { margin: 0; padding: 0; font-size: 13px; opacity: 0.6; color: #091d5c; margin:10px 0px;}
    input.form-control { margin: 0; padding: 9px; outline: none; box-shadow: none; border:1px solid #ccc;}
    ...
    return (
        ...
        {props.formik.values.product_selected != null &&
            <Content>
                ...
                <div key={props.formik.values.card_selected} className="mb-3">
                { (cards && cards.map(card => 
                    <Form.Check
                      custom
                      type={"checkbox"}
                      label={`Desejo utilizar meu cartão com final ${card.last_digits}`}
                      checked={(props.formik.values.card_selected && props.formik.values.card_selected.hash === card.hash)}
                      onChange={() => props.formik.setFieldValue('card_selected', card)}
                      id={`card-${card.hash}`}
                    />
                ))}
                    <Form.Check
                      custom
                      type={"checkbox"}
                      label={`Desejo informar os dados do cartão manualmente`}
                      checked={(props.formik.values.card_selected === null)}
                      onChange={() => props.formik.setFieldValue('card_selected', null)}
                      id={`card-none`}
                    />
                  </div>
            ...
            { props.formik.values.card_selected === null &&
                ...
                <button className="btn btn-primary" type="button" onClick={checkout}>Assinar agora</button>
            </Content>
        }
        </BlockUi>
    );
